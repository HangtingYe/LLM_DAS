...
if __name__ == "__main__":
    ...
    hard_anomaly_path = os.path.join(BASE_DIR, f'hard_anomalies/{tag}.npy')
    if os.path.exists(hard_anomaly_path):
        hard_anomalies = np.load(
            hard_anomaly_path,
            allow_pickle=True
        )
        legacy_hard_path = os.path.join(BASE_DIR, f'hard_anomalies/{legacy_tag}.npy')
        if not os.path.exists(legacy_hard_path):
            np.save(open(legacy_hard_path, 'wb'), hard_anomalies)
    else:
        hard_anomalies = generate_hard_anomalies(n_samples=int(X_train.shape[0]*0.1), model=model, X_train=X_train)
        np.save(open(hard_anomaly_path,'wb'), hard_anomalies)
    ...
    for i in range(runs):
        ...
        scaler = MinMaxScaler()
        scaler.fit(test_score.reshape(-1,1))
        ...
        # fit normalizer using train score
        scaler = MinMaxScaler()
        scaler.fit(test_score_original.reshape(-1,1))
        # normalize test score using the fitted normalizer
