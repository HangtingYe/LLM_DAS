...
def sanitize_scores(score):
    ...
    finite_vals = score[finite_mask]
    low = np.min(finite_vals)
    high = np.max(finite_vals)
    return np.nan_to_num(score, nan=low, posinf=high, neginf=low)
